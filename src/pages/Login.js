        ...
        <form onSubmit={handleSubmit}>
          {isRegistering && (
            <input type="text" name="name" placeholder="Full Name" onChange={handleChange} required 
              style={{width: '100%', padding: '12px', marginBottom: '15px', background: '#222', border: '1px solid #444', color: '#fff'}} />
          )}
          ...
          <button type="submit" style={{width: '100%', padding: '15px', background: '#C5A059', border: 'none', fontWeight: 'bold', cursor: 'pointer'}}>
            {isRegistering ? 'Register' : 'Login'}
          </button>
        </form>

        <p style={{textAlign: 'center', marginTop: '20px', color: '#888', cursor: 'pointer'}} onClick={() => setIsRegistering(!isRegistering)}>
